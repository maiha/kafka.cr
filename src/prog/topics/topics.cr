...
#  [not implemented] #{PROGRAM_NAME} --create --partitions=1 --replication-factor=1 --topic=topic1
  ...
  def do_show(topics)
    req = Kafka::Protocol::MetadataRequestV0.new(0, "kafka-topics", topics)
    res = execute req
    print_res(res)
  end
      ...
      if meta.error_code == 0
        if !consumer_offsets && meta.name == "__consumer_offsets"
          ...
          meta.partitions.each do |pm|
            if pm.error_code == 0
              puts "#{meta.name}##{pm.id}\tleader=#{pm.leader}, replicas=#{pm.replicas.inspect}, isrs=#{pm.isrs.inspect}"
            else
              err = Kafka::Protocol.errmsg(pm.error_code)
              puts "#{meta.name}##{pm.id}\t#{err}"
            end
          end
        end
      else
